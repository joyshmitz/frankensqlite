...
const BEAD_ID: &str = "bd-19hv";
const ISSUES_JSONL: &str = ".beads/issues.jsonl";
const UNIT_TEST_IDS: [&str; 2] = [
    "test_bd_19hv_unit_compliance_gate",
    "prop_bd_19hv_structure_compliance",
];
const PILLAR_CHECKLIST_IDS: [&str; 12] = [
    "test_unsafe_forbid_workspace",
    "test_clippy_pedantic_deny",
    "test_durability_bound_formula",
    "test_durability_bound_uses_p_upper",
    "test_repair_completeness_k_symbols",
    "test_repair_completeness_decode_proof",
    "test_eprocess_alarm_before_loss",
    "test_redundancy_autopilot_hardens",
    "test_behavioral_parity_golden_file",
    "test_wal_write_lock_eliminated",
    "test_cx_threading",
    "test_labruntime_deterministic_replay",
];
const E2E_TEST_IDS: [&str; 2] = ["test_e2e_bd_19hv", "test_e2e_bd_19hv_compliance"];
...
struct ComplianceEvaluation {
    ...
    missing_pillar_checklist_ids: Vec<&'static str>,
...
impl ComplianceEvaluation {
    fn is_compliant(&self) -> bool {
        self.missing_unit_ids.is_empty()
            && self.missing_pillar_checklist_ids.is_empty()
...
fn evaluate_description(description: &str) -> ComplianceEvaluation {
    ...
    let missing_pillar_checklist_ids = PILLAR_CHECKLIST_IDS
        .into_iter()
        .filter(|id| !contains_identifier(description, id))
        .collect::<Vec<_>>();
...
fn synthetic_compliant_description() -> String {
    ...
    for id in PILLAR_CHECKLIST_IDS {
        text.push_str("- ");
        text.push_str(id);
        text.push('\n');
    }
...
#[test]
fn test_bd_19hv_unit_compliance_gate() -> Result<(), String> {
    ...
    if !evaluation.missing_pillar_checklist_ids.is_empty() {
        return Err(format!(
            "bead_id={BEAD_ID} case=pillar_checklist_ids_missing missing={:?}",
            evaluation.missing_pillar_checklist_ids
        ));
    }
...
proptest! {
    #[test]
    fn prop_bd_19hv_structure_compliance(missing_index in 0usize..REQUIRED_TOKENS.len()) {
...
#[test]
fn test_e2e_bd_19hv_compliance() -> Result<(), String> {
    let description = load_issue_description(BEAD_ID)?;
    let evaluation = evaluate_description(&description);
    ...
    eprintln!(
        "INFO bead_id={BEAD_ID} case=e2e_summary missing_unit_ids={} missing_pillar_checklist_ids={} missing_e2e_ids={} missing_log_levels={} missing_log_standard_ref={}",
        evaluation.missing_unit_ids.len(),
        evaluation.missing_pillar_checklist_ids.len(),
    ...
    for id in &evaluation.missing_pillar_checklist_ids {
        eprintln!("WARN bead_id={BEAD_ID} case=missing_pillar_checklist_id id={id}");
    }
...
#[test]
fn test_e2e_bd_19hv() -> Result<(), String> {
    test_e2e_bd_19hv_compliance()
}
